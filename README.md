...
### 10. 🖥️ 系统运维与微服务监控中心 (`/ops`)
- **微服务全景资产与健康拓扑**：全景监控 8 大核心微服务（API 网关、统一认证、用户中心、内容中心、风控治理、消息推送、文件存储、运维服务）及 4 大中间件基础设施（MySQL 主从、Redis 哨兵、RabbitMQ、Nacos 集群）；
- **实时探活与探针详情抽屉 (`ServiceDetailDrawer`)**：支持对目标微服务发起实时心跳检测，直观查看服务元数据、部署环境、运行时长、端口、CPU 占用、JVM 堆内存与 HikariCP 数据库连接池实时快照；
- **告警事件中心与规则目录 (`AlertRulesModal`)**：聚合当前活动事件与历史已恢复告警，支持按严重等级（致命、严重、警告、提示）过滤与快速确认处置，内嵌告警规则目录弹窗查看检测阈值与持续时长；
- **JVM 性能大盘与动态监控策略 (`ResourcePolicyModal`)**：图形化展示各计算节点 CPU、物理内存与 JVM 堆分配水位，支持在线配置告警阈值、探针探测超时与自动轮询周期；
- **环境快速切换与离线容灾**：支持开发/测试/预发布/生产多环境视窗切换，未联通真实端点时自适应启用高拟真资产快照兜底。
...
│   │   │       ├── ResourcePolicyModal.tsx  # 资源告警阈值与监控策略配置
